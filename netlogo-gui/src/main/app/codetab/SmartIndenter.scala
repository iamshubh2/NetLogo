...
extends Indenter {
  ...
  case class TokenizedLine(lineNum: Int, lineStart: Int, lineEnd: Int, text: String, leadingSpaces: Int, tokens: Seq[Token]) {
    ...
    // bracketDelta lists the number of brackets closed on this line.
    // "" = 0, "[" = 0, "]" = 1, "[]" = 0, "]]" = 2, "[]]" = 1, &c
  ...
  private def executeIndentations(indentations: Seq[LineIndent]): Unit = {
    ...
    val offset = indentations.foldLeft(0) { (offset: Int, indent: LineIndent) =>
      val newOffset = (offset, indent) match {
        case (offset, AddIndent(lineNum, lineStart, _, text, delta)) =>
          builder.append(spaces(delta))
          builder.append(text)
          (offset + delta)
        case (offset, RemoveIndent(lineNum, lineStart, _, text, delta)) =>
          if (text.length > - delta) {
            builder.append(text.substring(- delta, text.length))
          }
          (offset + delta)
        case (offset, MaintainIndent(lineNum, lineStart, _, text)) =>
          builder.append(text)
          offset
      }
      builder.append("\n")
  ...
  private def lineIndentation(startOffset: Int, endOffset: Int): Seq[LineIndent] = {
    val startLine = code.offsetToLine(startOffset)
